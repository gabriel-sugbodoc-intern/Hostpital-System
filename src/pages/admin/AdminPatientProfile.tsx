...
export default function AdminPatientProfile() {
  ...
  const refreshPatient = async (forceSelectEncounterId?: string): Promise<PatientProfile | null> => {
  ...
  const handleSaveModal = async (formData: any) => {
    ...
    try {
      if (activeModal === 'new-encounter') {
        ...
        const res = await apiClient.saveSoapNote(patientId, selectedEncounterId, formData, editingRecord?._id ?? editingRecord?.id);
        if (res.error) { toast.error(res.error); return; }
        toast.success(editingRecord ? "SOAP note updated" : "SOAP note added");
        refreshEncounterRecords();
      } else if (activeModal === 'diagnosis') {
        const res = await apiClient.saveDiagnosis(patientId, selectedEncounterId, formData, editingRecord?._id ?? editingRecord?.id);
        if (res.error) { toast.error(res.error); return; }
        toast.success(editingRecord ? "Diagnosis updated" : "Diagnosis added");
        refreshEncounterRecords();
      } else if (activeModal === 'vital') {
        const res = await apiClient.saveVitalSigns(patientId, selectedEncounterId, formData, editingRecord?._id ?? editingRecord?.id);
        if (res.error) { toast.error(res.error); return; }
        toast.success(editingRecord ? "Vital signs updated" : "Vital signs recorded");
        refreshEncounterRecords();
      } else if (activeModal === 'procedure') {
        const res = await apiClient.saveProcedure(patientId, selectedEncounterId, formData, editingRecord?._id ?? editingRecord?.id);
        if (res.error) { toast.error(res.error); return; }
        toast.success(editingRecord ? "Procedure updated" : "Procedure added");
        refreshEncounterRecords();
      } else if (activeModal === 'prescription') {
        const res = await apiClient.savePrescription(patientId, selectedEncounterId, formData, editingRecord?._id ?? editingRecord?.id);
        if (res.error) { toast.error(res.error); return; }
        toast.success(editingRecord ? "Prescription updated" : "Prescription added");
        refreshEncounterRecords();
      } else if (activeModal === 'lab') {
        const res = await apiClient.saveLabResult(patientId, selectedEncounterId, formData, editingRecord?._id ?? editingRecord?.id);
        if (res.error) { toast.error(res.error); return; }
        toast.success(editingRecord ? "Lab result updated" : "Lab result saved");
        refreshEncounterRecords();
      } else if (activeModal === 'imaging') {
        const res = await apiClient.saveImagingRecord(patientId, selectedEncounterId, formData, editingRecord?._id ?? editingRecord?.id);
        if (res.error) { toast.error(res.error); return; }
  ...
  if (authError) {
    const isPendingIssue = authError.toLowerCase().includes('pending') || authError.toLowerCase().includes('confirm');
    return (
    ...
    <div className="space-y-6 animate-in slide-up">
      ...
      <Section title="Clinical Records (Encounter Details)">
        {!isDoctor && (
          <div className="flex items-center gap-2 rounded-xl border border-amber-200 bg-amber-50 p-3.5 text-amber-900 dark:border-amber-900/50 dark:bg-amber-950/30 dark:text-amber-200 mb-5 text-xs font-medium shadow-sm">
            ...
            <span><strong>Read-Only Mode:</strong> Medical record creation and modifications are restricted exclusively to authorized doctors. Admins and Patients may view all records.</span>
          </div>
        )}
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          <div className="bg-card border border-border rounded-xl p-5">
                    ...
                    >
                      <div className="flex items-center justify-between gap-1">
                        <span className="font-semibold text-xs text-foreground">
                          {encDate ? new Date(encDate).toLocaleDateString(undefined, { month: 'short', day: 'numeric', year: 'numeric' }) : 'Recent Encounter'}
                        </span>
                        <span className={`text-[10px] font-semibold px-1.5 py-0.5 rounded-full ${
                          encStatus === 'Completed' ? 'bg-emerald-100 text-emerald-700 dark:bg-emerald-950/40 dark:text-emerald-300' :
                          encStatus === 'In Progress' ? 'bg-blue-100 text-blue-700 dark:bg-blue-950/40 dark:text-blue-300' :
                      ...
                      <div className="text-xs text-muted-foreground mt-0.5">
                        {encounter.doctor ?? encounter.physician ?? 'Provider'} · {encounter.department ?? encounter.specialty ?? 'Clinical Care'}
                      </div>
            ...
            <div className="mt-3 pt-2 border-t border-border text-xs text-muted-foreground flex items-center gap-1"><Activity className="w-3 h-3" />Showing records linked to the selected encounter</div>
          </div>
          ...
          <InfoCard title="Encounter Details" icon={<Stethoscope className="w-4 h-4" />}>
            {selectedEncounter ? (
              <div className="space-y-3">
                ...
                <InfoRow label="Appointment" value={selectedEncounter.appointmentId ? `#${selectedEncounter.appointmentId.slice(0, 8)}` : '—'} />
                <InfoRow label="Encounter Type" value={selectedEncounter.encounterType ?? 'Outpatient Visit'} />
          ...
          <div className="bg-card border border-border rounded-xl p-5">
            ...
            {loadingEncounterRecords && <div className="mt-4 text-xs text-muted-foreground flex items-center gap-2"><Loader2 className="w-3.5 h-3.5 animate-spin text-primary" /> Loading selected encounter records…</div>}
          </div>
        </div>
          ...
          <RecordListCard
            ...
            headerAction={
              isDoctor ? (
                <button
                  type="button"
                  onClick={() => { setEditingRecord(null); setActiveModal('soap'); }}
                  className="inline-flex items-center gap-1 text-xs px-2.5 py-1 rounded-lg bg-primary text-primary-foreground hover:bg-primary/90 font-medium transition-colors"
                >
            ...
            onEditRecord={isDoctor ? (rec => { setEditingRecord(rec); setActiveModal('soap'); }) : undefined}
          ...
          <RecordListCard
            ...
            headerAction={
              isDoctor ? (
                <button
                  type="button"
                  onClick={() => { setEditingRecord(null); setActiveModal('diagnosis'); }}
                  className="inline-flex items-center gap-1 text-xs px-2.5 py-1 rounded-lg bg-primary text-primary-foreground hover:bg-primary/90 font-medium transition-colors"
                >
            ...
            onEditRecord={isDoctor ? (rec => { setEditingRecord(rec); setActiveModal('diagnosis'); }) : undefined}
            onDeleteRecord={isDoctor ? (recId => handleDeleteRecord('diagnosis', recId)) : undefined}
          >
            {record => <><InfoRow label="Diagnosis" value={record.diagnosis ?? record.description ?? record.desc ?? '—'} /><InfoRow label="ICD Code" value={record.icdCode ?? record.code ?? '—'} /><InfoRow label="Date" value={formatRecordDate(record)} /><InfoRow label="Provider" value={record.diagnosingPhysician ?? record.physician ?? '—'} /><InfoRow label="Status" value={record.status ?? '—'} /></>}
          </RecordListCard>
          ...
          <RecordListCard
            ...
            headerAction={
              isDoctor ? (
                <button
                  type="button"
                  onClick={() => { setEditingRecord(null); setActiveModal('vital'); }}
                  className="inline-flex items-center gap-1 text-xs px-2.5 py-1 rounded-lg bg-primary text-primary-foreground hover:bg-primary/90 font-medium transition-colors"
                >
          ...
          <RecordListCard
            ...
            headerAction={
              isDoctor ? (
                <button
                  type="button"
                  onClick={() => { setEditingRecord(null); setActiveModal('procedure'); }}
                  className="inline-flex items-center gap-1 text-xs px-2.5 py-1 rounded-lg bg-primary text-primary-foreground hover:bg-primary/90 font-medium transition-colors"
                >
            ...
            onEditRecord={isDoctor ? (rec => { setEditingRecord(rec); setActiveModal('procedure'); }) : undefined}
            onDeleteRecord={isDoctor ? (recId => handleDeleteRecord('procedure', recId)) : undefined}
          >
            {record => <><InfoRow label="Procedure" value={record.name ?? record.procedureName ?? record.type ?? record.description ?? '—'} /><InfoRow label="Date" value={formatRecordDate(record)} /><InfoRow label="Provider" value={record.doctor ?? record.physician ?? '—'} /><RecordText label="Notes" value={record.notes} /><InfoRow label="Status" value={record.status ?? '—'} /></>}
          </RecordListCard>
        </div>
      </Section>
        ...
        <RecordListCard
          ...
          headerAction={
            isDoctor ? (
              <button
                type="button"
                onClick={() => { setEditingRecord(null); setActiveModal('prescription'); }}
                className="inline-flex items-center gap-1 text-xs px-2.5 py-1 rounded-lg bg-primary text-primary-foreground hover:bg-primary/90 font-medium transition-colors"
              >
          ...
          onEditRecord={isDoctor ? (rec => { setEditingRecord(rec); setActiveModal('prescription'); }) : undefined}
          onDeleteRecord={isDoctor ? (recId => handleDeleteRecord('prescription', recId)) : undefined}
        >
          {record => <><InfoRow label="Medication" value={record.drug ?? record.medicationName ?? record.med ?? '—'} /><InfoRow label="Dosage" value={record.dosage ?? '—'} /><InfoRow label="Frequency" value={record.frequency ?? '—'} /><InfoRow label="Duration" value={record.duration ?? '—'} /><RecordText label="Instructions" value={record.instructions} /><InfoRow label="Prescriber" value={record.prescribingDoctor ?? record.doctor ?? '—'} /><InfoRow label="Status" value={record.status ?? '—'} /></>}
        </RecordListCard>
      </Section>
          ...
          <RecordListCard
            ...
            headerAction={
              isDoctor ? (
                <button
                  type="button"
                  onClick={() => { setEditingRecord(null); setActiveModal('lab'); }}
                  className="inline-flex items-center gap-1 text-xs px-2.5 py-1 rounded-lg bg-primary text-primary-foreground hover:bg-primary/90 font-medium transition-colors"
                >
            ...
            onEditRecord={isDoctor ? (rec => { setEditingRecord(rec); setActiveModal('lab'); }) : undefined}
            onDeleteRecord={isDoctor ? (recId => handleDeleteRecord('lab', recId)) : undefined}
          >
            {record => <><InfoRow label="Test" value={record.test_name ?? record.testName ?? record.test ?? '—'} /><InfoRow label="Result" value={record.result ?? record.resultSummary ?? record.value ?? '—'} /><InfoRow label="Reference Range" value={record.reference_range ?? record.referenceRange ?? record.range ?? '—'} /><InfoRow label="Released" value={record.dateReleased ?? formatRecordDate(record)} /><InfoRow label="Status" value={record.status ?? record.resultStatus ?? '—'} /></>}
          </RecordListCard>
          ...
          <RecordListCard
            ...
            headerAction={
              isDoctor ? (
                <button
                  type="button"
                  onClick={() => { setEditingRecord(null); setActiveModal('imaging'); }}
                  className="inline-flex items-center gap-1 text-xs px-2.5 py-1 rounded-lg bg-primary text-primary-foreground hover:bg-primary/90 font-medium transition-colors"
                >
      ...
      {activeModal && (
        <RecordEditModal
          ...
          onClose={() => { setActiveModal(null); setEditingRecord(null); }}
    ...
    <div className="bg-card border border-border rounded-xl p-5 space-y-4">
      <div className="flex items-center justify-between gap-2">
        <div className="flex items-center gap-2">
          <div className="text-primary">{icon}</div>
          <h3 className="font-semibold text-foreground">{title}</h3>
          {records.length > 0 && <span className="text-xs text-muted-foreground">({records.length})</span>}
        </div>
        {headerAction}
      </div>
      <div className="space-y-3">
        {records.length > 0 ? (
          records.map((record, index) => {
            ...
            return (
              <div key={recId} className="group relative space-y-3 border-t border-border pt-4 first:border-t-0 first:pt-0">
  ...
  const [formData, setFormData] = useState<Record<string, any>>(() => {
    if (!editingRecord) {
      if (modalType === 'new-encounter') {
        return { encounter_date: new Date().toISOString().slice(0, 10), department: 'General Medicine', chief_complaint: '', diagnosis: '', status: 'In Progress' };
      }
      if (modalType === 'vital') {
        return { blood_pressure: '', heart_rate: '', respiratory_rate: '', temperature: '', oxygen_saturation: '', height_cm: '', weight_kg: '' };
      }
      if (modalType === 'soap') {
      ...
      if (modalType === 'procedure') {
        return { name: '', code: '', notes: '', performed_at: new Date().toISOString().slice(0, 10) };
      }
      if (modalType === 'prescription') {
        return { drug: '', dosage: '', frequency: '', duration: '', instructions: '', status: 'Active' };
      }
      if (modalType === 'lab') {
      ...
      if (modalType === 'clinical-notes') {
        return { chief_complaint: '', history_of_present_illness: '', diagnosis: '', treatment_provided: '', follow_up_recommendations: '', encounter_notes: '', status: 'In Progress' };
      }
      return {};
    }
    return {
      ...editingRecord,
      encounter_date: editingRecord.encounter_date ?? editingRecord.date ?? new Date().toISOString().slice(0, 10),
      ...
      reference_range: editingRecord.reference_range ?? editingRecord.referenceRange ?? editingRecord.range ?? '',
      modality: editingRecord.modality ?? editingRecord.examinationName ?? editingRecord.type ?? '',
      blood_pressure: editingRecord.blood_pressure ?? editingRecord.bloodPressure ?? (editingRecord.systolic && editingRecord.diastolic ? `${editingRecord.systolic}/${editingRecord.diastolic}` : ''),
      ...
      name: editingRecord.name ?? editingRecord.procedureName ?? editingRecord.type ?? editingRecord.description ?? '',
    };
  });
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4 animate-in fade-in" role="dialog" aria-modal="true">
      <div className="max-h-[90vh] w-full max-w-2xl overflow-y-auto rounded-2xl border bg-card p-6 shadow-2xl">
        <div className="flex items-center justify-between gap-4 border-b pb-4">
          <div>
            <h2 className="text-lg font-bold text-foreground">{titles[modalType] ?? 'Edit Medical Record'}</h2>
            <p className="text-xs text-muted-foreground mt-0.5">Fill out the clinical record details. Changes will update the patient history.</p>
          </div>
        ...
        <form onSubmit={handleSubmit} className="mt-5 space-y-4">
          {modalType === 'new-encounter' && (
            <>
              {appointments && appointments.length > 0 && (
                <div>
                  <label className="block text-xs font-semibold text-foreground mb-1">Associated Patient Appointment</label>
                  <select
                    value={formData.appointment_id ?? ''}
                    onChange={e => {
                      const selectedId = e.target.value;
                      const selectedAppt = appointments.find((a: any) => String(a.id) === selectedId);
                      if (selectedAppt) {
                        setFormData(prev => ({
                          ...prev,
                          appointment_id: selectedId,
                          encounter_date: selectedAppt.date ?? selectedAppt.appointmentDate ?? prev.encounter_date,
                          department: selectedAppt.department ?? selectedAppt.specialty ?? prev.department,
                  ...
                  >
                    ...
                    {appointments.map((appt: any) => (
                      <option key={appt.id} value={appt.id}>
                        Appt #{String(appt.id).slice(0, 8)} · {appt.date ?? appt.appointmentDate ?? 'Date'} · {appt.department ?? appt.specialty ?? 'General'} ({appt.status})
                      </option>
                    ))}
                  </select>
                </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  <label className="block text-xs font-semibold text-foreground mb-1">Encounter Date *</label>
                  <input type="date" required value={formData.encounter_date ?? new Date().toISOString().slice(0, 10)} onChange={e => handleChange('encounter_date', e.target.value)} className="w-full rounded-lg border border-input bg-background px-3 py-2 text-sm" />
                </div>
                <div>
                  <label className="block text-xs font-semibold text-foreground mb-1">Encounter Time</label>
                  <input type="time" value={formData.encounter_time ?? '09:00'} onChange={e => handleChange('encounter_time', e.target.value)} className="w-full rounded-lg border border-input bg-background px-3 py-2 text-sm" />
                </div>
              </div>
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                <div>
                  <label className="block text-xs font-semibold text-foreground mb-1">Department / Specialty *</label>
                  <input type="text" required value={formData.department ?? ''} onChange={e => handleChange('department', e.target.value)} placeholder="e.g. Internal Medicine" className="w-full rounded-lg border border-input bg-background px-3 py-2 text-sm" />
                </div>
                <div>
                  <label className="block text-xs font-semibold text-foreground mb-1">Encounter Type</label>
                  <select value={formData.encounter_type ?? 'Outpatient Visit'} onChange={e => handleChange('encounter_type', e.target.value)} className="w-full rounded-lg border border-input bg-background px-3 py-2 text-sm">
                ...
                <div>
                  <label className="block text-xs font-semibold text-foreground mb-1">Encounter Status</label>
                  <select value={formData.status ?? 'In Progress'} onChange={e => handleChange('status', e.target.value)} className="w-full rounded-lg border border-input bg-background px-3 py-2 text-sm">
              ...
              <div>
                <label className="block text-xs font-semibold text-foreground mb-1">Chief Complaint *</label>
                <input type="text" required value={formData.chief_complaint ?? ''} onChange={e => handleChange('chief_complaint', e.target.value)} placeholder="e.g. Fever, persistent cough for 3 days" className="w-full rounded-lg border border-input bg-background px-3 py-2 text-sm" />
              </div>
              <div>
                <label className="block text-xs font-semibold text-foreground mb-1">Initial Working Diagnosis</label>
                <input type="text" value={formData.diagnosis ?? ''} onChange={e => handleChange('diagnosis', e.target.value)} placeholder="e.g. Acute Upper Respiratory Infection" className="w-full rounded-lg border border-input bg-background px-3 py-2 text-sm" />
              </div>
            </>
          )}
          ...
          {modalType === 'soap' && (
            <>
              <div>
                <label className="block text-xs font-semibold text-foreground mb-1">Subjective (S)</label>
                <textarea rows={3} value={formData.subjective ?? ''} onChange={e => handleChange('subjective', e.target.value)} placeholder="Patient's symptoms, history, concerns, chief complaint..." className="w-full rounded-lg border border-input bg-background p-3 text-sm" />
              </div>
              <div>
                <label className="block text-xs font-semibold text-foreground mb-1">Objective (O)</label>
                <textarea rows={3} value={formData.objective ?? ''} onChange={e => handleChange('objective', e.target.value)} placeholder="Physical exam findings, vital signs, lab/imaging values..." className="w-full rounded-lg border border-input bg-background p-3 text-sm" />
              </div>
              <div>
                <label className="block text-xs font-semibold text-foreground mb-1">Assessment (A)</label>
                <textarea rows={3} value={formData.assessment ?? ''} onChange={e => handleChange('assessment', e.target.value)} placeholder="Clinical impression, primary & differential diagnoses..." className="w-full rounded-lg border border-input bg-background p-3 text-sm" />
              </div>
              <div>
                <label className="block text-xs font-semibold text-foreground mb-1">Plan (P)</label>
                <textarea rows={3} value={formData.plan ?? ''} onChange={e => handleChange('plan', e.target.value)} placeholder="Treatment plan, medications prescribed, diagnostic orders, follow-up..." className="w-full rounded-lg border border-input bg-background p-3 text-sm" />
              </div>
            </>
          )}
          ...
          {modalType === 'diagnosis' && (
            <>
              <div>
                <label className="block text-xs font-semibold text-foreground mb-1">Diagnosis Description *</label>
                <input type="text" required value={formData.description ?? ''} onChange={e => handleChange('description', e.target.value)} placeholder="e.g. Essential (primary) hypertension" className="w-full rounded-lg border border-input bg-background px-3 py-2 text-sm" />
              </div>
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                <div>
                  <label className="block text-xs font-semibold text-foreground mb-1">ICD-10 Code</label>
                  <input type="text" value={formData.code ?? ''} onChange={e => handleChange('code', e.target.value)} placeholder="e.g. I10" className="w-full rounded-lg border border-input bg-background px-3 py-2 text-sm" />
                </div>
                <div>
                  <label className="block text-xs font-semibold text-foreground mb-1">Category</label>
                  <select value={formData.category ?? 'Primary'} onChange={e => handleChange('category', e.target.value)} className="w-full rounded-lg border border-input bg-background px-3 py-2 text-sm">
                ...
                <div>
                  ...
                  <select value={formData.status ?? 'Active'} onChange={e => handleChange('status', e.target.value)} className="w-full rounded-lg border border-input bg-background px-3 py-2 text-sm">
          ...
          {modalType === 'vital' && (
            <>
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
                <div>
                  <label className="block text-xs font-semibold text-foreground mb-1">Blood Pressure</label>
                  <input type="text" value={formData.blood_pressure ?? ''} onChange={e => handleChange('blood_pressure', e.target.value)} placeholder="120/80" className="w-full rounded-lg border border-input bg-background px-3 py-2 text-sm" />
                </div>
                <div>
                  <label className="block text-xs font-semibold text-foreground mb-1">Heart Rate (bpm)</label>
                  <input type="number" value={formData.heart_rate ?? ''} onChange={e => handleChange('heart_rate', e.target.value)} placeholder="72" className="w-full rounded-lg border border-input bg-background px-3 py-2 text-sm" />
                </div>
                <div>
                  <label className="block text-xs font-semibold text-foreground mb-1">Resp. Rate (/min)</label>
                  <input type="number" value={formData.respiratory_rate ?? ''} onChange={e => handleChange('respiratory_rate', e.target.value)} placeholder="16" className="w-full rounded-lg border border-input bg-background px-3 py-2 text-sm" />
                </div>
                <div>
                  <label className="block text-xs font-semibold text-foreground mb-1">Temp (°C)</label>
                  <input type="number" step="0.1" value={formData.temperature ?? ''} onChange={e => handleChange('temperature', e.target.value)} placeholder="36.8" className="w-full rounded-lg border border-input bg-background px-3 py-2 text-sm" />
                </div>
                <div>
                  <label className="block text-xs font-semibold text-foreground mb-1">O₂ Saturation (%)</label>
                  <input type="number" value={formData.oxygen_saturation ?? ''} onChange={e => handleChange('oxygen_saturation', e.target.value)} placeholder="98" className="w-full rounded-lg border border-input bg-background px-3 py-2 text-sm" />
                </div>
                <div>
                  <label className="block text-xs font-semibold text-foreground mb-1">Weight (kg)</label>
                  <input type="number" step="0.1" value={formData.weight_kg ?? ''} onChange={e => handleChange('weight_kg', e.target.value)} placeholder="65" className="w-full rounded-lg border border-input bg-background px-3 py-2 text-sm" />
                </div>
              </div>
            </>
          )}
          ...
          {modalType === 'procedure' && (
            <>
              <div>
                <label className="block text-xs font-semibold text-foreground mb-1">Procedure Name *</label>
                <input type="text" required value={formData.name ?? ''} onChange={e => handleChange('name', e.target.value)} placeholder="e.g. Nebulization, Wound Debridement" className="w-full rounded-lg border border-input bg-background px-3 py-2 text-sm" />
              </div>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  <label className="block text-xs font-semibold text-foreground mb-1">Procedure Code</label>
                  <input type="text" value={formData.code ?? ''} onChange={e => handleChange('code', e.target.value)} placeholder="e.g. CPT-94640" className="w-full rounded-lg border border-input bg-background px-3 py-2 text-sm" />
                </div>
                <div>
                  <label className="block text-xs font-semibold text-foreground mb-1">Performed Date</label>
                  <input type="date" value={formData.performed_at ?? ''} onChange={e => handleChange('performed_at', e.target.value)} className="w-full rounded-lg border border-input bg-background px-3 py-2 text-sm" />
                </div>
              </div>
              <div>
                <label className="block text-xs font-semibold text-foreground mb-1">Procedure Notes & Details</label>
                <textarea rows={3} value={formData.notes ?? ''} onChange={e => handleChange('notes', e.target.value)} placeholder="Clinical procedure notes, patient tolerance..." className="w-full rounded-lg border border-input bg-background p-3 text-sm" />
              </div>
            </>
          )}
          ...
          {modalType === 'prescription' && (
            <>
              <div>
                <label className="block text-xs font-semibold text-foreground mb-1">Medication / Drug Name *</label>
                <input type="text" required value={formData.drug ?? ''} onChange={e => handleChange('drug', e.target.value)} placeholder="e.g. Amoxicillin, Paracetamol" className="w-full rounded-lg border border-input bg-background px-3 py-2 text-sm" />
              </div>
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                <div>
                  <label className="block text-xs font-semibold text-foreground mb-1">Dosage</label>
                  <input type="text" value={formData.dosage ?? ''} onChange={e => handleChange('dosage', e.target.value)} placeholder="e.g. 500 mg" className="w-full rounded-lg border border-input bg-background px-3 py-2 text-sm" />
                </div>
                <div>
                  <label className="block text-xs font-semibold text-foreground mb-1">Frequency</label>
                  <input type="text" value={formData.frequency ?? ''} onChange={e => handleChange('frequency', e.target.value)} placeholder="e.g. Every 8 hours" className="w-full rounded-lg border border-input bg-background px-3 py-2 text-sm" />
                </div>
                <div>
                  <label className="block text-xs font-semibold text-foreground mb-1">Duration</label>
                  <input type="text" value={formData.duration ?? ''} onChange={e => handleChange('duration', e.target.value)} placeholder="e.g. 7 days" className="w-full rounded-lg border border-input bg-background px-3 py-2 text-sm" />
                </div>
              </div>
              <div>
                <label className="block text-xs font-semibold text-foreground mb-1">Special Instructions</label>
                <input type="text" value={formData.instructions ?? ''} onChange={e => handleChange('instructions', e.target.value)} placeholder="Take after meals with plenty of water" className="w-full rounded-lg border border-input bg-background px-3 py-2 text-sm" />
              </div>
            </>
          )}
          ...
          {modalType === 'lab' && (
            <>
              <div>
                <label className="block text-xs font-semibold text-foreground mb-1">Lab Test Name *</label>
                <input type="text" required value={formData.test_name ?? ''} onChange={e => handleChange('test_name', e.target.value)} placeholder="e.g. Complete Blood Count (CBC), Fasting Blood Sugar" className="w-full rounded-lg border border-input bg-background px-3 py-2 text-sm" />
              </div>
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                <div>
                  <label className="block text-xs font-semibold text-foreground mb-1">Result Value</label>
                  <input type="text" value={formData.result ?? ''} onChange={e => handleChange('result', e.target.value)} placeholder="e.g. 110" className="w-full rounded-lg border border-input bg-background px-3 py-2 text-sm" />
                </div>
                <div>
                  <label className="block text-xs font-semibold text-foreground mb-1">Unit</label>
                  <input type="text" value={formData.unit ?? ''} onChange={e => handleChange('unit', e.target.value)} placeholder="e.g. mg/dL" className="w-full rounded-lg border border-input bg-background px-3 py-2 text-sm" />
                </div>
                <div>
                  <label className="block text-xs font-semibold text-foreground mb-1">Reference Range</label>
                  <input type="text" value={formData.reference_range ?? ''} onChange={e => handleChange('reference_range', e.target.value)} placeholder="e.g. 70-100" className="w-full rounded-lg border border-input bg-background px-3 py-2 text-sm" />
                </div>
              </div>
            </>
          )}
          ...
          {modalType === 'imaging' && (
            <>
              <div>
                <label className="block text-xs font-semibold text-foreground mb-1">Imaging Examination / Modality *</label>
                <input type="text" required value={formData.modality ?? ''} onChange={e => handleChange('modality', e.target.value)} placeholder="e.g. Chest X-Ray PA/Lateral, Brain CT Scan" className="w-full rounded-lg border border-input bg-background px-3 py-2 text-sm" />
              </div>
              <div>
                <label className="block text-xs font-semibold text-foreground mb-1">Body Part / Facility</label>
                <input type="text" value={formData.body_part ?? ''} onChange={e => handleChange('body_part', e.target.value)} placeholder="e.g. Chest, Abdomen" className="w-full rounded-lg border border-input bg-background px-3 py-2 text-sm" />
              </div>
              <div>
                <label className="block text-xs font-semibold text-foreground mb-1">Findings Summary</label>
                <textarea rows={3} value={formData.findings ?? ''} onChange={e => handleChange('findings', e.target.value)} placeholder="Radiologist / Physician findings..." className="w-full rounded-lg border border-input bg-background p-3 text-sm" />
              </div>
              <div>
                <label className="block text-xs font-semibold text-foreground mb-1">Impression</label>
                <textarea rows={2} value={formData.impression ?? ''} onChange={e => handleChange('impression', e.target.value)} placeholder="Diagnostic impression..." className="w-full rounded-lg border border-input bg-background p-3 text-sm" />
              </div>
            </>
          )}
          ...
          {modalType === 'clinical-notes' && (
            <>
              <div>
                <label className="block text-xs font-semibold text-foreground mb-1">Chief Complaint</label>
                <input type="text" value={formData.chief_complaint ?? ''} onChange={e => handleChange('chief_complaint', e.target.value)} className="w-full rounded-lg border border-input bg-background px-3 py-2 text-sm" />
              </div>
              <div>
                <label className="block text-xs font-semibold text-foreground mb-1">History of Present Illness (HPI)</label>
                <textarea rows={2} value={formData.history_of_present_illness ?? ''} onChange={e => handleChange('history_of_present_illness', e.target.value)} className="w-full rounded-lg border border-input bg-background p-3 text-sm" />
              </div>
              <div>
                <label className="block text-xs font-semibold text-foreground mb-1">Diagnosis Summary</label>
                <input type="text" value={formData.diagnosis ?? ''} onChange={e => handleChange('diagnosis', e.target.value)} className="w-full rounded-lg border border-input bg-background px-3 py-2 text-sm" />
              </div>
              <div>
                <label className="block text-xs font-semibold text-foreground mb-1">Treatment Provided</label>
                <textarea rows={2} value={formData.treatment_provided ?? ''} onChange={e => handleChange('treatment_provided', e.target.value)} className="w-full rounded-lg border border-input bg-background p-3 text-sm" />
              </div>
              <div>
                <label className="block text-xs font-semibold text-foreground mb-1">Follow-up Recommendations</label>
                <textarea rows={2} value={formData.follow_up_recommendations ?? ''} onChange={e => handleChange('follow_up_recommendations', e.target.value)} className="w-full rounded-lg border border-input bg-background p-3 text-sm" />
              </div>
            </>
          )}

          <div className="flex items-center justify-end gap-3 pt-4 border-t mt-6">
            <button type="button" onClick={onClose} disabled={isSaving} className="px-4 py-2 text-sm font-medium rounded-lg border border-input bg-background hover:bg-muted transition-colors text-foreground">
              Cancel
            </button>
            <button type="submit" disabled={isSaving} className="inline-flex items-center gap-2 px-5 py-2 text-sm font-semibold rounded-lg bg-primary text-primary-foreground hover:bg-primary/90 transition-colors disabled:opacity-50">
