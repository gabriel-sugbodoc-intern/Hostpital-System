        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
          <div className="space-y-1">
            ...
            <div className="flex items-center gap-3 text-xs text-muted-foreground flex-wrap">
              ...
              {invoice.orderNo && <span className="font-mono bg-primary/10 text-primary px-2 py-0.5 rounded">Order: {invoice.orderNo}</span>}
              {invoice.pickupBranch && <span className="text-muted-foreground">Store: {invoice.pickupBranch}</span>}
              {invoice.deliveryAddress && <span className="text-muted-foreground truncate max-w-[200px]" title={invoice.deliveryAddress}>Delivery: {invoice.deliveryAddress}</span>}
...
export default function Billing() {
  ...
  const loadData = () => {
    ...
    Promise.all([apiClient.getAccountData(), apiClient.getPaymentHistory()]).then(([accountResult, historyResult]) => {
      ...
      const billByInvoice = new Map<string, Invoice>(bills.map((bill: Invoice) => [bill.invoiceNo, bill]));
      ...
      const transactionInvoiceNos = new Set(transactions.map((transaction: { invoiceNo: string }) => transaction.invoiceNo));
