                ...
                <div className="flex gap-2 mt-1">
                  {profile.allergies
                    ...
                    .map((a) => (
                      <span
                        key={a}
                        className="px-2 py-0.5 bg-warning/10 text-warning text-xs font-medium rounded-full border border-warning/20"
                      >
                        {a}
                      </span>
                    ))}
            ...
            {[
              {
                ...
                desc: "Get notified before visits",
              },
              ...
              <div key={item.id} className="flex items-center justify-between">
                ...
                <button
                  onClick={() =>
                    setNotifs({ ...notifs, [item.id]: !notifs[item.id as keyof typeof notifs] })
                  }
                  className={`w-11 h-6 rounded-full transition-colors relative ${notifs[item.id as keyof typeof notifs] ? "bg-primary" : "bg-muted"}`}
                >
                  <div
                    className={`absolute top-1 left-1 bg-white w-4 h-4 rounded-full transition-transform ${notifs[item.id as keyof typeof notifs] ? "translate-x-5" : "translate-x-0"}`}
                  ></div>
                </button>
              </div>
            ))}
